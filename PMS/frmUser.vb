...
Public Class frmUser
    ...
    Public isUserEmployee As Boolean
    ...
    Private Sub BindToText()
        Try
            sqL = "SELECT *,  (Lastname + ', ' + Firstname + ' ' + Middlename) as TName FROM USERS s LEFT JOIN EMployee t ON s.EmployeeId = t.EmployeeId  WHERE Username ='" & ListView1.FocusedItem.Text & "'"
            ConnDB()
            cmd = New SqlCommand(sqL, conn)
            dr = cmd.ExecuteReader()
            ...
            If dr.Read = True Then
                ...
                txtPassword.Text = Decrypt(dr("Upassword"))
                txtConfirmPass.Text = Decrypt(dr("Upassword"))
                cmbRole.Text = dr("Role").ToString
                txtEmployee.Text = dr("TName").ToString
                txtEmployee.Tag = dr("EmployeeId").ToString
    ...
    Private Function ValidateOldPassword() As Boolean
        ...
        Try
            ...
            If dr.Read = True Then
                ...
                If Decrypt(dr("Upassword")) = txtOldPassword.Text Then
                    isValid = True
                End If
            End If
    ...
    Private Sub AddUsers()
        ...
        Try
            sqL = "INSERT INTO USERS(Username, Upassword, Role, EmployeeId) VALUES('" & txtUsername.Text & "', '" & strPassword & "', '" & cmbRole.Text & "', '" & txtEmployee.Tag & "')"
            ConnDB()
            cmd = New SqlCommand(sqL, conn)
            Dim i As Integer
    ...
    Private Sub UpdateUser()
        Dim strPassword As String = Encrypt(txtPassword.Text)
        Try
            sqL = "UPDATE Users SET Upassword = '" & strPassword & "', Role = '" & cmbRole.Text & "', EmployeeId = '" & txtEmployee.Tag & "' WHERE Username = '" & txtUsername.Text & "'"
            ConnDB()
            cmd = New SqlCommand(sqL, conn)
            Dim i As Integer
            i = cmd.ExecuteNonQuery
